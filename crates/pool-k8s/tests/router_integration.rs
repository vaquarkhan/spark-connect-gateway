...
fn stickiness_holds_across_membership_changes() {
    ...
    for _ in 0..5 {
        assert_eq!(
            router.resolve_session(&key).unwrap(),
            first,
...
#[test]
fn new_session_after_repopulation_picks_from_new_set() {
    let pool = Arc::new(K8sPool::new());
    let router = Router::new(pool.clone(), Arc::new(StubStore::default()));

    // Pool empty at first.
    let early = SessionKey::new("u", "early");
    assert!(router.resolve_session(&early).is_none());

    // Watcher fires → new sessions can now route.
    pool.set_backends(vec!["a:1".into(), "b:1".into()]);
    let late = SessionKey::new("u", "late");
    let chosen = router.resolve_session(&late).unwrap();
